kmplot <- function(km, mark=3, simple=FALSE,
  ...
  ## graphic parameters
  if(!add){
    par(list(oma=c(1,1,.1,.1), mar=c(4+ng,4+extra.left.margin,0,0)+.1))
    if(simple) par( mar=c(3,4,0,0)+.1 )
    par( list(...) )
  }
  ...
  if(!simple){
    ...
    mtext( group.names, side=1, line=line.pos, at=group.name.pos, adj=1, font=2, col=1, las=1, cex=cex.axis )
    ...
    for( i in seq(ng) ){
      ...
      mtext( side=1, at=this$time+w.adj, text=this$n.risk, line=line.pos[i], cex=cex.axis, font=2, adj=1, col=1, las=1)
    }
