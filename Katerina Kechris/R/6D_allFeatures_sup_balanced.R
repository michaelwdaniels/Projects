...
load("saved_images/6D_boxplots_balanced")
# ---------------------------------------------------------------------------- #
# Script 6D all balanced
# Author: Mike Daniels
#
...
which(apply(feature.table,2,sum)/3500<0.05)
names(feature.table)[c(17,20,21)]
feature.table <- feature.table[,-c(17,20,21)]
########################
...
for(g in 1:length(contam.perc)){
  ...
  for(j in 1:n1){
    ...
    for(i in 1:l){
      ...
      repeat{
        if(product(apply(training[,c(2,5,7,9,10,13,19)],2,sum))!=0 & product(apply(testing[,c(2,5,7,9,10,13,19)],2,sum))!=0) {
     ...
     # labels[inTrain]
      ...
      if(g<2){
        ...
        repeat{
          if(product(apply(divided.data$train[,c(2,5,7,9,10,19)],2,sum))!=0 &
             product(apply(divided.data$test[,c(2,5,7,9,10,19)],2,sum))!=0 ) {
            print(paste0("Seed = ",seed1," doesn't contain a column of zeros."))
            break
          } else{ 
            seed1 <- 10000 + seed1 + 21
            set.seed(seed1)
            divided.data = divideYeastData(feature.table, date.table, labels,
                                           train.class = 'positive',
                                           num.train.examples = train.size, seed = seed1, constant = constant, negatives = 'match')
  ...
  png(paste0('results/plots/6D_allFeatures_balanced_',contam.perc[g]*100,'.png'),
  ...
  png(paste0('results/plots/6D_allFeatures_balanced_',contam.perc[g]*100,'_paper.png'),
      height = 8, width = 8, res = 300, units = 'in')
...
save.image("saved_images/6D_boxplots_balanced")
