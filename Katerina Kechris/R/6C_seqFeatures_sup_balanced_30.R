...
load("saved_images/6C_boxplots_balanced_30")
# ---------------------------------------------------------------------------- #
...
for(j in 1:n1){
  ...
  for(i in 1:l){
    ...
    all.preds.J48$new = predict(J48.model,testing) # predictions of tree model on test set
...
png(paste0('results/plots/6C_seqFeatures_balanced_',contam.perc[g]*100,'.png',
    ...
    boxplot(tog$AUC~tog$method*tog$n,at=at,col=c('red','lightblue1','cadetblue3','dodgerblue2'),xlab='% Training Set', ylab='AUC',xaxt="n",
        main=paste0('Supervised vs Semi-supervised (14 Sequence-derived Features)\nAUC for various training % with 25 iterations\nTest: balanced (',contam.perc[g]*100,'% Contamination)'),ylim=c(0.5,1))
...
png(paste0('results/plots/6C_seqFeatures_balanced_',percent.contam[g]*100,'_paper.png',
           height = 8, width = 8, res = 300, units = 'in'))
    boxplot(tog$AUC~tog$method*tog$n,at=at,col=c('red','lightblue1','cadetblue3','dodgerblue2'),xlab='% Training Set', ylab='AUC',xaxt="n",ylim=c(0.5,1))
            axis(side=1,at=seq(2.5,97.5,5),labels=floor(p*3500))
            legend("topleft",c("Semi-supervised","Supervised-SVM","Supervised-Lasso","Supervised-Tree"),col=c('red','lightblue1','cadetblue3','dodgerblue2'),pch=15)
            dev.off()
            
}

save.image("saved_images/6C_boxplots_balanced_2")
