...
load("saved_images/6C_boxplots_all")
# ---------------------------------------------------------------------------- #
# Script 6C
# Author: Mike Daniels
#
...
for(j in 1:n1){
  ...
  for(i in 1:l){
    ...
    labels.pos <- which(labels==1) # collection of row numbers from labels which are positive
    labels.neg <- which(labels==0) # collection of row numbers from labels which are negative
    inTrain.pos <- sample(1:length(labels.pos),size = floor(p[i]*length(labels.pos)), replace = FALSE)
    inTrain.neg <- sample(1:length(labels.neg),size = floor(p[i]*length(labels.neg)), replace = FALSE)
    inTrain <- c(inTrain.pos,inTrain.neg)
...
for(j in 1:n1){
  ...
  for(i in 1:l){
    ...
    repeat{
      if(product(apply(divided.data$train[,c(1,2,3,4,5,6,7)],2,sum))!=0){
...
png('results/plots/6C-ROC_seqFeatures_pos_only_boxplots_post_post_2002_all.png',
...
png('results/plots/6C-ROC_seqFeatures_pos_only_boxplots_post_post_2002_0_100.png',
    height = 8, width = 8, res = 300, units = 'in')
boxplot(tog$AUC~tog$method*tog$n,at=c(1,2,3,4,6,7,8,9,11,12,13,14,16,17,18,19,21,22,23,24,26,27,28,29,31,32,33,34,36,37,38,39,41,42,43,44),col=c('red','lightblue1','cadetblue3','dodgerblue2'),xlab='% Training Set', ylab='AUC',xaxt="n",
...
png('results/plots/6C-ROC_seqFeatures_pos_only_boxplots_post_post_2002_paper.png',
    height = 8, width = 8, res = 300, units = 'in')
boxplot(tog$AUC~tog$method*tog$n,at=c(1,2,3,4,6,7,8,9,11,12,13,14,16,17,18,19,21,22,23,24,26,27,28,29,31,32,33,34,36,37,38,39,41,42,43,44),col=c('red','lightblue1','cadetblue3','dodgerblue2'),xlab='% Training Set', ylab='AUC',xaxt="n",ylim=c(0.50,0.75))
axis(side=1,at=seq(2.5,42.5,5),labels=p*100)
legend("topleft",c("Semi-supervised","Supervised-SVM","Supervised-Lasso","Supervised-Tree"),col=c('red','lightblue1','cadetblue3','dodgerblue2'),pch=15)
dev.off()

save.image("saved_images/6C_boxplots_all")
