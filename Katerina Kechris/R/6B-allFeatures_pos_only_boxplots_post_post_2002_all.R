...
load("saved_images/6B_boxplots_all")
# ---------------------------------------------------------------------------- #
# Script 6B
# Author: Mike Daniels
#
...
cat('SCRIPT 6B: Train + only (varying n - post 2002), test +/- genes post 2002, all features\n')
...
# Train on positive genes only, test on pos and matched neg genes to training size
...
for(j in 1:n1){
  ...
  for (train.size in c(700,600,500,400,300,200,100,50,25)){
    ...
    repeat{
      if(product(apply(divided.data$train[,c(2,3,5,7,9,10,13,17,20,21,22)],2,sum))!=0){
...
png('results/plots/6B-ROC_allFeatures_pos_only_boxplots_post_post_2002_all.png',
...
png('results/plots/6B-ROC_allFeatures_pos_only_boxplots_post_post_2002_all_paper.png',
    height = 8, width = 8, res = 300, units = 'in')
...
save.image("saved_images/6B_boxplots_all")
