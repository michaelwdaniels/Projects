load("saved_images/5B_boxplots_all")
# ---------------------------------------------------------------------------- #
# Script 5B
# Author: Mike Daniels
#
...
load('R/cerevisiae_preprocessed.RData')
...
for(j in 1:n1){
  ...
  for (train.size in c(60,55,50,45,40,35,30,25,20)){
    ...
    divided.data = divideYeastData(feature.table, date.table, labels,
                                   ...
                                   num.train.examples = train.size, seed = seed1, negatives = 'all')
    repeat{
...
png('results/plots/5B-ROC_allFeatures_pos_only_boxplots_pre_post_2002_all.png',
...
png('results/plots/5B-ROC_allFeatures_pos_only_boxplots_pre_post_2002_all_paper.png',
    height = 8, width = 8, res = 300, units = 'in')
...
save.image('saved_images/5B_boxplots_all')
