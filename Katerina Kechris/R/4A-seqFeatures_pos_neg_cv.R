...
cat('SCRIPT 4A: Train +/- (varying n), test +/- genes, seq features\n')
...
# Save AUC info
write.table(aucs[,c(2:4,1)], 'results/tables/4A-seqFeatures_AUCs.txt', sep = '\t',
            row.names = F, quote = F)

# Save performance info
write.table(all.perf, 'results/tables/4A-seqFeatures_pos_neg_lcmix_perf.txt',
            sep = '\t', row.names = F, quote = F)

# Plot AUCs
cols = brewer.pal(11, 'Spectral')
png('results/plots/4A-barplot_seqFeatures_AUCs.png', height = 8, width = 15, 
    res = 300, units = 'in')
par(mfrow = c(1,4))
