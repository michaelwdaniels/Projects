load("saved_images/5B_boxplots_match")
# ---------------------------------------------------------------------------- #
# Script 5B
# Author: Mike Daniels
#
...
for(j in 1:n1){
  ...
  for (train.size in c(60,55,50,45,40,35,30,25,20)){
    ...
    divided.data = divideYeastData(feature.table, date.table, labels,
                                   ...
                                   num.train.examples = train.size, seed = seed1, negatives = 'match')
    repeat{
      if(product(apply(divided.data$train[,1:7],2,sum))!=0&(product(apply(divided.data$train[,1:7],1,sum))!=0|train.size>30)){
        ...
        divided.data = divideYeastData(feature.table, date.table, labels,
                                       ...
                                       num.train.examples = train.size, seed = seed1, contant = constant, negatives = 'match')
      }
    }   
    
    is.train = divided.data$is.train
    lcmix.preds = getLCMIXmodels.windows(feature.table, is.train, labels[!is.train])$preds
...
png('results/plots/5B-ROC_seqFeatures_pos_only_boxplots_pre_post_2002_match.png',
...
png('results/plots/5B-ROC_seqFeatures_pos_only_boxplots_pre_post_2002_match_paper.png',
    height = 8, width = 8, res = 300, units = 'in')
...
save.image('saved_images/5B_boxplots_match')
