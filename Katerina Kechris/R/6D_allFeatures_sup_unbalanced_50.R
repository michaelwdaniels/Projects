...
load("saved_images/6D_boxplots_unbalanced_30")
# ---------------------------------------------------------------------------- #
...
for(g in 3:length(contam.perc)){
  ...
  for(j in 1:n1){
    ...
    for(i in 1:l){
      ...
      all.labels.svm$new = labels.contam[-inTrain] # collects true labels in corresponding list to be used to produce ROC
      ...
      predict.J48 <- predict(J48.model,testing)
      labels.J48 <- labels.contam[-inTrain]
      ...
      all.labels.sup$new = labels.contam[-inTrain] # collects true labels in corresponding list to be used to produce ROC
      ...
      if(g<2){
        ...
        repeat{
          if(product(apply(divided.data$train[,c(1,2,3,4,5,6,7)],2,sum))!=0 &
             product(apply(divided.data$test[,c(1,2,3,4,5,6,7)],2,sum))!=0 ) {
  ...
  AUC2 <- t(AUC.svm)
  if(g<2){
  AUC1 <- t(AUC.semi)} else{AUC1 <- t(AUC1.semi)}
  AUC3 <- t(AUC.sup)
  AUC4 <- t(AUC.J48)
  ...
  tog <- data.frame(AUC=AUC.list[[g-2]],n=n,method=method)
  box.at <- seq(1:(l*5))
  ...
  boxplot(tog$AUC~tog$method*tog$n,at=at,col=c('red','lightblue1','cadetblue3','dodgerblue2'),xlab='% Training Set', ylab='AUC',xaxt="n",
          main=paste0('Supervised vs Semi-supervised (All 22 Features)\nAUC for various training % with 25 iterations\nTest: unbalanced (',contam.perc[g]*100,'% Contamination)'),ylim=c(0.5,1))
  axis(side=1,at=seq(2.5,92.5,5),labels=floor(p*3500))
  ...
  png(paste0('results/plots/6D_allFeatures_unbalanced_',contam.perc[g]*100,'_paper.png'),
      height = 8, width = 8, res = 300, units = 'in')
  boxplot(tog$AUC~tog$method*tog$n,at=at,col=c('red','lightblue1','cadetblue3','dodgerblue2'),xlab='% Training Set', ylab='AUC',xaxt="n",ylim=c(0.5,1))
  axis(side=1,at=seq(2.5,92.5,5),labels=floor(p*3500))
  legend("topleft",c("Semi-supervised","Supervised-SVM","Supervised-Lasso","Supervised-Tree"),col=c('red','lightblue1','cadetblue3','dodgerblue2'),pch=15)
  dev.off()
...
save.image("saved_images/6D_boxplots_unbalanced_2")
