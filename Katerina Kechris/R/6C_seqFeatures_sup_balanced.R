...
load("saved_images/6C_boxplots_balanced")
# ---------------------------------------------------------------------------- #
...
idx <- which(apply(feature.table[,1:7],2,sum)/3500<0.2)
...
AUC.RandomForest <- matrix(0,ncol=n1,nrow=l) # collects AUC values for decision tree analysis
...
rownames(AUC.RandomForest) <- rownames(AUC.svm) <- rownames(AUC.sup) <- rownames(AUC.semi) <- p # row names are training percent levels, columns will be iterations
...
for(g in 1:length(contam.perc)){
  ...
  for(j in 1:n1){
    ...
    all.preds.RandomForest = list() # list of predictions
    all.labels.RandomForest = list() # list of labels being predicted
    ...
    for(i in 1:l){
      ...
      repeat{
        if(product(apply(training[,1:7],2,sum))!=0 & product(apply(testing[,1:7],2,sum))!=0) {
      ...
      # Decision Tree Random Forest (try like a 100 trees)
      training.tree <- cbind(as.factor(labels.contam[inTrain]),training)
      colnames(training.tree)[1] <- 'labels'
      RandomForest.model = randomForest(labels~., data = training.tree)
      predict.RandomForest <- predict(RandomForest.model,testing,type='class') # when type = "class", a factor vector is returned. When type = "prob", a matrix of confidence values is returned (one column per class).
      labels.RandomForest <- labels[idx.test]
      
      all.preds.RandomForest$new = predict.RandomForest # predictions of tree model on test set
      names(all.preds.RandomForest)[length(all.preds.RandomForest)] = paste0('Tree (train n = ', p[i]*length(labels), ')') # assign names to predictions in a list
      all.labels.RandomForest$new = labels.RandomForest # collects true labels in corresponding list to be used to produce ROC
      names(all.labels.RandomForest)[length(all.labels.RandomForest)] = paste0('Tree (train n = ', p[i]*length(labels), ')') # assign names to labels in a list
      ...
      if(g<2){
        ...
        repeat{
          if(product(apply(divided.data$train[,1:7],2,sum))!=0 &
             product(apply(divided.data$test[,1:7],2,sum))!=0 ) {
    ...
    RandomForest.pred <- prediction-class(all.preds.RandomForest,all.labels.RandomForest)
    plot(performance-class(RandomForest.pred,measure="prec", x.measure="rec"),col="red")
    ...
    ROC.RandomForest <- multiroc(all.preds.RandomForest,all.labels.RandomForest) # collection of ROC calculations from prediction and label lists
    ...
    for(k in 1:l){
      ...
      AUC.RandomForest[k,j] <- ROC.RandomForest[[k]]$auc # extracts AUC values from ROC and places them in a matrix to be used in boxplots
  ...
  png(paste0('results/plots/6C_seqFeatures_balanced_',contam.perc[g]*100,'.png'),
  ...
  boxplot(tog$AUC~tog$method*tog$n,at=at,col=c('red','lightblue1','cadetblue3','dodgerblue2'),xlab='Training Set', ylab='AUC',xaxt="n",
          main=paste0('Supervised vs Semi-supervised (Sequeance Derived Features)\nAUC for various training % with 25 iterations\nTest: balanced (',contam.perc[g]*100,'% Contamination)'),ylim=c(0.45,1))
  ...
  png(paste0('results/plots/6C_seqFeatures_balanced_',contam.perc[g]*100,'_paper.png'),
      height = 8, width = 8, res = 300, units = 'in')
...
save.image("saved_images/6C_boxplots_balanced")
